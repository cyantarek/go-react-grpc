...
dockerize-all: dockerize-frontend dockerize-envoy dockerize-backend
...
kubernetes-redeploy:
	kubectl patch deployment todo-backend-deployment -p "{\"spec\":{\"template\":{\"metadata\":{\"labels\":{\"build\":\"$(date +%s)\"}}}}}"
	kubectl patch deployment todo-frontend-deployment -p "{\"spec\":{\"template\":{\"metadata\":{\"labels\":{\"build\":\"$(date +%s)\"}}}}}"
	kubectl patch deployment todo-envoy-deployment -p "{\"spec\":{\"template\":{\"metadata\":{\"labels\":{\"build\":\"$(date +%s)\"}}}}}"